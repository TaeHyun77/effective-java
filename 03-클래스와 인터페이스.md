...
2. 비정적 멤버 클래스
    ...
    > 뷰 : 원본 데이터의 전체가 아닌 일부분을 보여주는 또는 특정 관점에서 보여주는 객체
    > 
    
    Ex) Map 인터페이스의 구현체들은 보통 keySet, entrySet, values 메서드가 반환하는 자신의 컬렉션 뷰를 구현할 때 비정적 멤버 클래스를 사용
    
    또한, Set과 List 같은 다른 컬렉션 인터페이스 구현들도 자신의 반복자를 구현 시 비정적 멤버 클래스를 사용<br><br>
    ...
    public class MyMap<K, V> {
        ...
        public class KeySet extends AbstractSet<K> {
            ...
            public Iterator<K> iterator() {
                return new Iterator<K>() {
                    int index = 0;
                    public boolean hasNext() { return index < entries.size(); }
                    public K next() { return entries.get(index++).getKey(); }
                };
            }
        ...
        public Set<K> keySet() {
            return new KeySet(); // 외부 클래스(MyMap) 인스턴스를 참조함
        }
    }
    ```
    <br>
    
    멤버 클래스에서 바깥 인스턴스에 접근할 일이 없다면 static을 붙여서 정적 멤버 클래스로 만들자
    
    static을 생략한다면 바깥 인스턴스 인스턴스를 암묵적으로 참조하는데, 이 참조를 저장하기 위한 시간과 공간이 소비되고, 가비지 컬렉션이 바깥 클래스의 인스턴스를 수거하지 못하게 되어 메모리 누수가 발생할 수 있음
    
    ⇒ 참조가 눈에 보이지 않으니 문제의 원인을 찾기 힘듦<br><br>
    
    private 정적 멤버 클래스는 흔히 바깥 클래스가 표현하는 객체의 한 부분( 구성 요소 )을 나타낼 때 사용
    
    많은 Map 구현체는 각각의 키-값 쌍을 표현하는 Entry 객체들을 가지고 있으며 모든 Entry가 Map과 연관되어 있지만, Entry 메서드들을 맵을 직접 사용하지 않음
    
    즉, Entry를 정적으로 선언하는게 좋으며, 오히려 비정적으로 선언하면 불필요한 외부 참조가 생길 수 있음<br><br>
    
3. 익명 클래스
    
    익명 클래스는 이름이 없으며, 바깥 클래스의 멤버도 아님
    
    멤버와 달리 쓰이는 시점에 선언과 동시에 인스턴스가 만들어짐
    
    오직 비정적인 문맥에서 사용될 때만 바깥 클래스의 인스턴스를 참조할 수 있으며, 정적 문맥에서라도 상수 변수 이외의 정적 멤버는 가질 수 없음 
    
    → 상수 표현을 위해 초기화 된 final 기본 타입과 문자열 필드만 가질 수 있음
    ...
    class Outer {
        ...
        void method() {
            Runnable r = new Runnable() {
                public void run() {
                    System.out.println(x); // 바깥 클래스의 인스턴스 참조 가능
                }
            };
        }
    }
    ...
    class Outer {
        static void staticMethod() {
            Runnable r = new Runnable() {
                public void run() {
                    // System.out.println(x); // 오류: static 문맥이므로 Outer 인스턴스를 참조할 수 없음
                }
            };
        }
    }
    ...
    **익명 클래스는 응용 시 제약이 많음**
    ...
    선언한 지점에서만 인스턴스를 만들 수 있고, 이름이 없기에 instanceOf 검사나 클래스의 이름이 필요한 작업은 수행할 수 없으며, 인터페이스를 구현하는 동시에 다른 클래스를 상속할 수도 없음
    
    클라이언트는 익명 클래스가 상위 타입에서 상속한 멤버 외에는 호출할 수 없으며, 익명 클래스는 표현식 중간에 위치하므로 짧지 않다면 가독성이 떨어지게 됨<br><br>
    
    **익명 클래스는 정적 팩터리 메서드를 구현할 때 사용됨**
    ...
    public class GreetingFactory {
        public static Greeting koreanGreeting() {
            return new Greeting() {
                @Override
        ...
        public static Greeting englishGreeting() {
            return new Greeting() {
                @Override
                public void sayHello() {
...
4. 지역 클래스
    ...
    메서드, 생성자 또는 초기화 블록 내부에서 정의된 클래스로, 네 가지 중첩 클래스 중 가장 드물게 사용됨
    
    즉, 클래스 정의가 메서드 몸체 안에 들어 있음
...
멤버 클래스의 인스턴스 각각이 바깥 인스턴스를 참조한다면 비정적으로, 그렇지 않다면 정적으로 생성하자

중첩 클래스가 한 메서드 안에서만 쓰이거나 그 인스턴스를 생성하는 지점이 단 한 곳이고 해당 타입으로 쓰기에 적합한 클래스나 인터페이스가 이미 있다면 익명 클래스로 만들고 그렇지 않다면 지역 클래스로 만들자<br><br>

### 탑 레벨 클래스는 한 파일에 담자

---

하나의 소스 파일에 여러 개의 탑 레벨 클래스를 선언하는 것은 딱히 문제가 되지 않지만 ( public 클래스는 딱 하나만 가능하며 그 이름은 파일 이름과 같아야 함 ), 동일한 이름의 클래스가 서로 다른 소스 파일에 정의되어 있을 경우 컴파일 순서에 따라 어느 클래스 정의가 사용될지 예측할 수 없음<br><br>

이를 해결하기 위해서는 단순히 탑 레벨 클래스들을 서로 다른 소스 파일로 분리하면 됨

굳이 여러 탑 레벨 클래스를 한 파일에 담고 싶다면 정적 멤버 클래스를 사용하는 방식을 사용하자
